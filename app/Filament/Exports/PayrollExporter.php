<?php

namespace App\Filament\Exports;

use App\Models\Payroll;
use Filament\Actions\Exports\ExportColumn;
use Filament\Actions\Exports\Exporter;
use Filament\Actions\Exports\Models\Export;

class PayrollExporter extends Exporter
{
    protected static ?string $model = Payroll::class;

    public static function getColumns(): array
    {
        return [
            ExportColumn::make('employee.name'),
            ExportColumn::make('code'),
            ExportColumn::make('from'),
            ExportColumn::make('to'),
            ExportColumn::make('basic_salary_hours'),
            ExportColumn::make('basic_salary_amount'),
            ExportColumn::make('reg_ot_hours'),
            ExportColumn::make('reg_ot_amount'),
            ExportColumn::make('rd_ot_hours'),
            ExportColumn::make('rd_ot_amount'),
            ExportColumn::make('pag_ibig'),
            ExportColumn::make('sss'),
            ExportColumn::make('philhealth'),
            ExportColumn::make('total_deductions'),
            ExportColumn::make('total_earnings'),
            ExportColumn::make('status'),
        ];
    }

    public static function getCompletedNotificationBody(Export $export): string
    {
        $body = 'Your payroll export has completed and ' . number_format($export->successful_rows) . ' ' . str('row')->plural($export->successful_rows) . ' exported.';

        if ($failedRowsCount = $export->getFailedRowsCount()) {
            $body .= ' ' . number_format($failedRowsCount) . ' ' . str('row')->plural($failedRowsCount) . ' failed to export.';
        }

        return $body;
    }
}
